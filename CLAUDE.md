...
- `OmniKit` (SPM library): the engine + indexer.
  ...
  - `OmniImageEncoder`: Qwen3-VL ViT + merger, feature injection (images, scanned PDFs).
  - `OmniAudioEncoder`: Qwen2.5-Omni audio tower + projector, injected into the same backbone.
  - `WeightStore`: loads HF safetensors, merges retrieval LoRA in fp32, keeps the backbone bf16
    by default (`OMNI_BACKBONE_DTYPE` / `OMNI_BACKBONE_BF16=0` for fp32).
