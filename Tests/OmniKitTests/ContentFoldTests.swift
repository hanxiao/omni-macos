...
final class ContentFoldTests: XCTestCase {
    ...
    private func assertEveryFileAnswersForItself(_ store: VectorStore, files: Int, _ label: String,
        ...
        for g in 0 ..< 5 {
            let paths = Set(store.search(vec(7 + g), topK: 20).filter { $0.score > 0.99 }.map(\.path))
            XCTAssertEqual(paths.count, files / 5,
                           "\(label): shared content \(g) is held by \(paths.count) files, not \(files / 5)",
                           file: file, line: line)
        }
    }
